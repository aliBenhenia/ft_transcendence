...
class SECURITY(models.Model):
    ...
    def send_to_email(self, state, to):
        
        print(f"TO : {to}")
        if state:
            subject = 'Reset Your Password'
            message = f'Here is Your Code : {self.code} to Reset Your Password!'
        else:
            subject = '2FA Verification Code'
            message = f'Here is Your 2FA Code : {self.code_2fa} to Secure Login!'
        print(f"TO : {to}")
        r = send_mail(subject, message, '', [to], fail_silently=False)
        if (r > 0):
