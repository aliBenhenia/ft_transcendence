...
def list_conversation(request):
    ...
    messages = MESSAGES.objects.filter(Q(account=account) | Q(sender=account)).order_by('-time')
    if not messages:
...
def send_message(request):
    ...
    if not account_status(account, obj):
        return Response({'error': ERROR[6]}, status=400)
