...
def blocked_list(request):
    ...
    if is_twofactor(account):
        return Response({'error' : ERROR_MSG['15'], '2FA' : True}, status=401)
    block = BLOCKER.objects.filter(blocker=account)
    if not block.first():
    ...
    for b in block:
        information = {
            ...
            'picture': b.blocked.photo_url,
            'full_name': f'{b.blocked.first_name} {b.blocked.last_name}',
        }
        data.append(information)
    return Response({'vide': False, 'list' : data}, status=200)
